...
def pending_list(request):
	if request.user.is_superuser:
		prof=Profile.objects.get(user=request.user)
		pending_passes=Gatepass.objects.all().exclude(Q(hostel_warden="Rejected") | Q(hostel_assistant_warden="Rejected") \
			| Q(hostel_supervisor="Rejected") |Q(control_room="Rejected")).filter(Q(hostel_supervisor="Pending")| \
			Q(hostel_warden="Pending") & Q(hostel_assistant_warden="Pending") | Q(control_room="Pending")).order_by('-id')
...
def rejected_list(request):
	if request.user.is_superuser:
		prof=Profile.objects.get(user=request.user)
		rejected_passes=Gatepass.objects.all().filter(Q(hostel_supervisor="Rejected") | Q(hostel_warden="Rejected") | Q(hostel_assistant_warden="Rejected") | Q(control_room="Rejected")).order_by('-id')
		page = request.GET.get('page', 1)
...
def approved_list(request):
	if request.user.is_superuser:
		prof=Profile.objects.get(user=request.user)
		approved_passes=Gatepass.objects.all().exclude(Q(hostel_warden="Rejected") | Q(hostel_assistant_warden="Rejected")).filter(Q(hostel_supervisor="Approved")& (Q(hostel_warden="Approved") | Q(hostel_assistant_warden="Approved")) & Q(control_room="Approved")).order_by('-id')
